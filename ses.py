...
import pickle
from statsmodels.tsa.holtwinters import SimpleExpSmoothing as sme
...
def build_model(df, path):
    ...
    model_ses = sme(train)
    ses_model_result = model_ses.fit(optimized=True, use_brute=True)

    file = open(path + 'ses_model.pkl', 'wb')
    pickle.dump(ses_model_result, file)
    file.close()
    st.success('Model built and saved successfully')

    st.write(ses_model_result.summary())
    return ses_model_result


def load_model(path):
    model = pickle.load(open(path + 'ses_model.pkl', 'rb'))
    st.success('Model loaded successfully')
    st.write(model.summary())
    return model


def ses_model(df, path):
...
def app():
    ...
    model = ses_model(df, config['model_path'])
    if model:
        prediction(df, model)
